...
%------------------------------------------------------------------------------
	...
	if size(vector,2)==2
		...
		for classs=1:size(classes,3)
			[vcol symbol color]=randColorAndType([] , [], classs);
	...
	if size(vector,2)==3
		...
		for classs=1:size(classes,3)
			...
			plot3(means(1,classs), means(2,classs),means(3,classs), 'ro', 'MarkerSize', 15, 'LineWidth',3)
			vleg=cat(1, vleg, ['media #',num2str(classs)]);
			x=[means(1,classs),vector(1)];
			y=[means(2,classs),vector(2)];
		...
		plot3(vector(1), vector(2),vector(3), 'ks', 'MarkerSize', 20, 'LineWidth',5)
		legend(vleg)
	end
